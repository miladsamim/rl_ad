...
class SACAgent:
    ...
    def get_action_deterministically(self, state):
        action_probabilities = self.get_action_probabilities(state)
        discrete_action = np.argmax(action_probabilities[0])
        return self.action_space[discrete_action]
    ...
    def train_on_transition(self, state, discrete_action, next_state, reward, done):
        ...
        transition = (state, self.action_space.index(discrete_action), reward, next_state, done)
    ...
    def train_networks(self, transition):
        ...
        if len(self.replay_buffer) >= self.REPLAY_BUFFER_BATCH_SIZE:
            ...
            minibatch = random.sample(self.replay_buffer, self.REPLAY_BUFFER_BATCH_SIZE)
            minibatch_separated = list(map(list, zip(*minibatch)))

            # unravel transitions to get states, actions, rewards and next states
            states_tensor = torch.FloatTensor(np.array(minibatch_separated[0]))
            actions_tensor = torch.tensor(np.array(minibatch_separated[1]),dtype=torch.int64)
            rewards_tensor = torch.FloatTensor(np.array(minibatch_separated[2]))
            next_states_tensor = torch.FloatTensor(np.array(minibatch_separated[3]))
    ...
    def critic_loss(self, states_tensor, actions_tensor, rewards_tensor, next_states_tensor, done_tensor):
        with torch.no_grad():
            ...
            next_q_values_target = self.critic_target(next_states_tensor)
            next_q_values_target2 = self.critic_target2(next_states_tensor)
    ...
    def get_action_info(self, states_tensor):
        action_probabilities = self.actor_local(states_tensor)
    ...
    def get_action_probabilities(self, state):
        state = state.transpose(2,0,1)/255.0
        state_tensor = torch.tensor(state.copy(), dtype=torch.float32).unsqueeze(0)
        action_probabilities = self.actor_local(state_tensor)
